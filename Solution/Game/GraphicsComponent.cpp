...
void GraphicsComponent::Update(float aDeltaTime)
{
	...
	if (myEntity.GetShouldRotate() == true)
	{
		...
		{
			myEntity.myOrientation = CU::Matrix44f::CreateRotateAroundX(0.25f * aDeltaTime)* myEntity.myOrientation;
		}
		if (myEntity.GetShouldRotateY() == true)
		{
			myEntity.myOrientation = CU::Matrix44f::CreateRotateAroundY(0.25f * aDeltaTime) * myEntity.myOrientation;
		}
		if (myEntity.GetShouldRotateZ() == true)
		{
			myEntity.myOrientation = CU::Matrix44f::CreateRotateAroundZ(0.25f * aDeltaTime)* myEntity.myOrientation;
		}
