...
AKRESULT CAkStmTask::EnsureFileIsOpen()
{
	...
	if ( m_pDeferredOpenData && !m_bIsToBeDestroyed )
	{
		...
		else
		{
			...
			if ( m_pDeferredOpenData->bByString )
			{
				OS_PRINTF( szMsg, ( AK_FileNotFound == eResult ) ? AKTEXT("File not found: %s") : AKTEXT("Cannot open file: %s"),  m_pDeferredOpenData->pszFileName );
				AK::Monitor::PostString( szMsg, AK::Monitor::ErrorLevel_Error );
			}
			else
			{
				OS_PRINTF( szMsg, ( AK_FileNotFound == eResult ) ? AKTEXT("File not found: %u") : AKTEXT("Cannot open file: %u"),  m_pDeferredOpenData->fileID );
				AK::Monitor::PostString( szMsg, AK::Monitor::ErrorLevel_Error );
			}
#endif
		}
