...
AKRESULT AkDeferredOpenData::Execute( AkFileDesc & io_fileDesc )
{
	...
	if ( eResult != AK_Success )
	{
		...
		if ( bByString )
		{
			OS_PRINTF( szMsg, ( AK_FileNotFound == eResult ) ? AKTEXT("File not found: %s") : AKTEXT("Cannot open file: %s"),  pszFileName );
			AK::Monitor::PostString( szMsg, AK::Monitor::ErrorLevel_Error );
		}
		else
		{
			OS_PRINTF( szMsg, ( AK_FileNotFound == eResult ) ? AKTEXT("File not found: %u") : AKTEXT("Cannot open file: %u"),  fileID );
			AK::Monitor::PostString( szMsg, AK::Monitor::ErrorLevel_Error );
		}
	}
#endif
    ...
    )
{
    ...
    if ( eRes != AK_Success )
    {
        ...
        // Monitor error.
		AkOSChar szMsg[ AK_MAX_PATH + 64 ];
		OS_PRINTF( szMsg, ( AK_FileNotFound == eRes ) ? AKTEXT("File not found: %s") : AKTEXT("Cannot open file: %s"),  in_pszFileName );
		AK::Monitor::PostString( szMsg, AK::Monitor::ErrorLevel_Error );
#endif
        return eRes;
    }
