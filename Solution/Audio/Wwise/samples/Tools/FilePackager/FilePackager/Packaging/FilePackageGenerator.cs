...
namespace FilePackager.Packaging
{
    ...
    public class FilePackageGenerator : IProgressNotificationsDispatcher
    {
        ...
        public class Context
        {
            ...
            {
                public AK.Wwise.InfoFile.Platform Platform { get; set; }
        ...
        private void Generate( Context.GlobalInfo globalInfo, Context.PackageInfo packageInfo, Results results )
        {
            ...
            try
            {
                ...
                using (FileStream file = new FileStream(packageInfo.FilePackageFilename, FileMode.Create))
                {
                    ...
                    switch (globalInfo.Platform)
                    {
                        case AK.Wwise.InfoFile.Platform.Windows:
                        case AK.Wwise.InfoFile.Platform.Mac:
                        case AK.Wwise.InfoFile.Platform.VitaSW:
                        case AK.Wwise.InfoFile.Platform.VitaHW:
                        case AK.Wwise.InfoFile.Platform._3DS:
                        case AK.Wwise.InfoFile.Platform.iOS:
                        case AK.Wwise.InfoFile.Platform.Android:
                        case AK.Wwise.InfoFile.Platform.PS4:
                        case AK.Wwise.InfoFile.Platform.XboxOne:
                        case AK.Wwise.InfoFile.Platform.Linux:
                        case AK.Wwise.InfoFile.Platform.WindowsPhone:
            ...
            Results results)
        {
            ...
            foreach (AK.Wwise.InfoFile.FileDescriptorType soundbank in descriptors.OfType<AK.Wwise.InfoFile.SoundBank>())
            {
            ...
            foreach (AK.Wwise.InfoFile.FileDescriptorType stream in descriptors.OfType<AK.Wwise.InfoFile.File>())
            {
                if (!streamsLUT.Add(stream, mapLanguageIDs))
                {
                    NotifyLogMsg("Missing stream file: " + stream.ShortName + " (" + stream.Path + ")", Severity.Warning);
                    results.HasMissingFiles = true;
                }
            }
            streamsLUT.Sort();
            ...
            organizer.AddLUT(FileOrganizer.FileType.SoundBank, banksLUT);
            organizer.AddLUT(FileOrganizer.FileType.StreamedAudio, streamsLUT);
            organizer.AddLUT(FileOrganizer.FileType.ExternalSource, externalLUT);
        ...
        static internal Dictionary<string, uint> FindAllLanguages(IEnumerable<AK.Wwise.InfoFile.FileDescriptorType> descriptors )
        {
            ...
            foreach (AK.Wwise.InfoFile.File streamedFile in descriptors.OfType<AK.Wwise.InfoFile.File>())
            {
            ...
            foreach (AK.Wwise.InfoFile.SoundBank soundBank in descriptors.OfType<AK.Wwise.InfoFile.SoundBank>())
            {
