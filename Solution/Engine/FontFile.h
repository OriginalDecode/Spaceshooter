...
namespace Prism
{
	...
	struct fontChar
	{
		...
		{
			assert(texWidth && texHeight && "Texture width or height is zero");
	...
	bool ParseFontFile(std::vector<fontChar> &fontChars, const std::string &fontfilename, uint texwidth, uint texheight);
}